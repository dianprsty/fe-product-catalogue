...
const DetailProduct = () => {
  ...
  return (
    <div
      className="md:w-8/12 shadow-lg mx-auto flex gap-4 lg:flex-row flex-col 
      py-10 border lg:mt-10 bg-white"
    >
      {!isLoading ? (
        <>
          {" "}
          <div className="lg:w-6/12 w-full justify-center">
            <img
          ...
          <div className="flex flex-col gap-2 lg:w-6/12 w-full px-8 sm:px-20 lg:px-0">
            {product.is_diskon === 1 && (
              <p className="p-2 pr-10 bg-red-300 text-red-700 self-end font-semibold">
                {getDiskonPercentage(product)}% Off
              </p>
            )}
            <p className="text-2xl font-bold w-10/12">{product.nama}</p>
            <p className="text-sm py-2">
              Category
              <span className="text-xs text-green-600 bg-green-200 p-1 rounded-md ml-2 border border-green-500">
                {product.category}
              </span>
            </p>
            {product.is_diskon === 1 && (
            ...
            <div>
              ...
              <p className="text-sm w-10/12 text-justify">
